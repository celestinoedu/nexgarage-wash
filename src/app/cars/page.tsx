...
export default function VehiclesPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.from("vehicles").insert({ store_id: String(form.get("store_id") ?? vehicles.store.id), customer_id: String(form.get("customer_id")), plate: String(form.get("plate")).trim().toUpperCase(), make: String(form.get("make") ?? "") || null, model: String(form.get("model") ?? "") || null, color: String(form.get("color") ?? "") || null, size: String(form.get("size") ?? "") || null });
