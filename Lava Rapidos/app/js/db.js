...
// ---- Movimentos de colaborador (Vales e Pagamentos) ------------------------
export const colaboradorMovimentos = {
  list: (limit = 300) =>
    supabase
      .from("colaborador_movimentos")
  ...
  byFuncionario: (fid) =>
    supabase
      ...
      .order("data", { ascending: false })
      .then(ok),
  create: (row) => supabase.from("colaborador_movimentos").insert(row).select().single().then(ok),
  remove: (id) => supabase.from("colaborador_movimentos").delete().eq("id", id).then(ok),
};
