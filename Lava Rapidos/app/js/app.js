...
async function viewDashboard() {
  ...
  // Últimos atendimentos: considerar os últimos 29 dias.
  const limite = new Date();
  limite.setDate(limite.getDate() - 29);
  const limiteStr = limite.toISOString().slice(0, 10);
  const ultimosAts = ats.filter((a) => String(a.data).slice(0, 10) >= limiteStr);
  ...
  $("#view").innerHTML = `
    ...
    ${collapsibleCard("ultimos", `<h3>🕒 Últimos atendimentos</h3>`, tableAtend(ultimosAts))}
...
async function viewFuncionarios() {
  const [list, movs] = await Promise.all([
    ...
    db.colaboradorMovimentos.list(300),
  ]);

  // Saldo devedor por colaborador = total de Vales − total de Pagamentos.
  const saldo = (fid) =>
    movs
      .filter((m) => m.funcionario_id === fid)
      .reduce((s, m) => s + (m.tipo === "Vale" ? Number(m.valor || 0) : -Number(m.valor || 0)), 0);

  $("#view").innerHTML = `
    <div class="toolbar"><div></div>
      <div class="row gap">
        <button class="btn ok" id="addVale">+ Vale</button>
        <button class="btn primary" id="add">+ Funcionário</button>
      </div>
    </div>
    <div class="table-wrap"><table>
      <thead><tr><th>Nome</th><th>Telefone</th><th>Status</th><th class="r">Vale em aberto</th><th></th></tr></thead>
      <tbody>${list
        .map((f) => {
          const s = saldo(f.id);
          return `<tr><td><strong>${esc(f.nome)}</strong></td><td>${esc(f.telefone || "—")}</td>
          <td>${f.ativo ? '<span class="tag ok">ativo</span>' : '<span class="tag">inativo</span>'}</td>
          <td class="r">${s > 0 ? `<strong class="warn-txt">${money(s)}</strong>` : '<span class="muted">—</span>'}</td>
          <td class="r"><button class="btn small ghost" data-edit="${f.id}">Editar</button></td></tr>`;
        })
        .join("")}</tbody></table></div>

    ${card(`
      <div class="card-head"><h3>💸 Vales e pagamentos</h3></div>
      <div class="table-wrap"><table>
        <thead><tr><th>Data</th><th>Colaborador</th><th>Tipo</th><th>Observação</th><th class="r">Valor</th><th></th></tr></thead>
        <tbody>${
          movs.length
            ? movs
                .slice(0, 100)
                .map(
                  (m) => `<tr>
              <td>${dateBR(m.data)}</td>
              <td>${esc(m.funcionarios?.nome || m.funcionario_nome || "—")}</td>
              <td>${m.tipo === "Vale" ? '<span class="tag warn">vale</span>' : '<span class="tag ok">pagamento</span>'}</td>
              <td>${esc(m.observacao || "")}</td>
              <td class="r">${money(m.valor)}</td>
              <td class="r"><button class="btn small ghost" data-delmov="${m.id}">✕</button></td>
            </tr>`
                )
                .join("")
            : '<tr><td colspan="6" class="muted">Nenhum vale ou pagamento registrado.</td></tr>'
        }</tbody></table></div>`)}`;

  $("#add").onclick = () => formFunc();
  $("#addVale").onclick = () => formVale(list);
  $$("[data-edit]").forEach((b) => (b.onclick = () => formFunc(list.find((x) => x.id === b.dataset.edit))));
  $$("[data-delmov]").forEach((b) => (b.onclick = async () => {
    if (await confirmDialog("Excluir este lançamento?")) {
      await db.colaboradorMovimentos.remove(b.dataset.delmov); toast("Excluído."); route();
    }
  }));
}

// Registra um Vale (adiantamento) ou Pagamento para um colaborador.
function formVale(funcs) {
  const ativos = funcs.filter((f) => f.ativo !== false);
  const opcoes = (ativos.length ? ativos : funcs)
    .map((f) => `<option value="${f.id}">${esc(f.nome)}</option>`)
    .join("");
  const { close } = openModal("Registrar vale", `
    <form id="f" class="form">
      <label>Colaborador
        <select name="funcionario_id" required>${opcoes || '<option value="">— sem funcionários —</option>'}</select></label>
      <label>Tipo
        <select name="tipo"><option value="Vale">Vale (adiantamento)</option><option value="Pagamento">Pagamento (quitação)</option></select></label>
      ...
      <label>Observação<input name="observacao" placeholder="Ex: adiantamento quinzena" /></label>
  ...
  $("#f").onsubmit = async (e) => {
    ...
    const d = formData(e.target);
    d.valor = Number(d.valor || 0);
    const func = funcs.find((x) => x.id === d.funcionario_id);
    d.funcionario_nome = func?.nome || null;
    try { await db.colaboradorMovimentos.create(d); toast("Vale registrado."); close(); route(); }
    catch (err) { toast(err.message, "err"); }
  };
}
...
async function viewFinanceiro() {
  ...
  // O mês financeiro vai do dia 15 ao dia 14 do mês seguinte.
  const [ty, tm, td] = today().split("-").map(Number);
  let sy = ty, sm = tm;
  if (td < 15) { sm -= 1; if (sm === 0) { sm = 12; sy -= 1; } }
  let ey = sy, em = sm + 1;
  if (em === 13) { em = 1; ey += 1; }
  const pad = (n) => String(n).padStart(2, "0");
  const cicloIni = `${sy}-${pad(sm)}-15`;
  const cicloFim = `${ey}-${pad(em)}-14`;
  const mes = `${sy}-${pad(sm)}`;
  const doMes = list.filter((l) => {
    const d = String(l.data).slice(0, 10);
    return d >= cicloIni && d <= cicloFim;
  });
